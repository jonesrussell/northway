...
Status: [bounded operator-only ingestion is implemented](ingestion.md); no publisher collection is activated or scheduled. [The broader personal pilot](personal-feeds.md) covers development, entertainment, Canada, First Nations and world news; its [ten exact sources are owner-selected](source-approval-2026-08-31.md) for personal metadata-only use but remain disabled pending publisher-use, availability and activation gates. The owner approved the [five-source metadata-only PHP pilot](pilot-readiness.md) and future hourly Pi HTTPS polling on 2026-08-31. Read-only device and endpoint checks are complete; exact hardware measurements remain in private infra evidence. The current candidate filesystem is SD-backed, so storage/recovery validation remains a deployment gate. No scheduled polling is enabled. The earlier 1 GiB budget is a hypothesis, not a measured runtime requirement.
