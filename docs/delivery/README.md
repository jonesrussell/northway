...
GitHub Actions is configured, but [the initial contracts job](https://github.com/jonesrussell/northway/actions/runs/33347934014) did not start: GitHub reports an account billing/spending restriction. Dependabot success is not a successful contracts run. No billing setting was changed, and payment is not a prerequisite for local implementation. A runnable hosted or separately approved trusted runner and required-check enforcement remain an operational gap tracked with the runtime harness work.

No branch protection could be verified for this private repository (GitHub returned HTTP 403). This setup did not configure branch protection or required checks. Focused PRs and review evidence are documented workflow, not enforced controls. Establish review/required-check protection with a runnable CI path in the runtime harness issue.

Do not disable tests to work around account restrictions. Record local command evidence in PRs while the remote gate is unavailable. Do not attach a broadly privileged self-hosted runner to the home Pi as an automatic workaround.
