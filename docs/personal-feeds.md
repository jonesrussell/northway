...
Resolve cross-category conflicts in the saved category order: Development, Entertainment, Canada, First Nations, World initially. For each category, choose its highest-ranked eligible item whose canonical URL is unused and whose publisher group remains below the cap; try the next eligible item in that same category before declaring a gap. Earlier categories win a shared-publisher or duplicate conflict; this tie-break is explicit, stable and part of the preference revision, not a claim of equal realized coverage. Reordering requires an explicit preference revision.
...
- When several sources are due, use a persisted round-robin cursor over stable source IDs across the approved active set, with one attempt per source per pass. Skip sources not yet due or held by cache/Retry-After instructions. Advance the cursor after an admitted attempt even if it fails; persist its next position across restarts and budget exhaustion. If the next due source cannot reserve its configured response allowance, stop and resume there when capacity returns rather than repeatedly favoring small feeds or the first bootstrap source. Bootstrap and new candidates have equal scheduling priority; hourly/four-hour intervals are minimum spacing, not guaranteed service. This bounds repeated selection without claiming equal byte allocation or guaranteed freshness; #12 must test exhaustion/restart fairness with a large synthetic bootstrap feed.
