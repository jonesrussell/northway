...
identity.Operator is an explicit trusted-code escape hatch for local provisioning and future internal jobs, not authentication. Never call it with a tenant taken from HTTP, model output or another untrusted payload. File-access operators already have access to every tenant; SQLite and Go package boundaries do not defend against malicious trusted application code. Future jobs must derive tenant authority from their persisted, validated owner, carry it through batches, and recheck policy when starting work. Do not retain a service principal across requests/jobs as an authorization cache. No scheduler, batch endpoint, customer cache or feedback mutation exists yet; each must extend the isolation matrix when implemented.
...
The file and directory are synced before inserting the digest. A crash in between can leave an inactive credential file. This is not a transaction across filesystem and database: reconcile uncertain outcomes before retrying, and never overwrite an existing key file. Ordinary provisioning failures remove the newly created file; existing files are untouched. A stdout failure after successful creation does not remove the credential. stdout contains only status, tenant ID and the nonsecret lookup ID. Move the credential through your secret-management process into Claudriel's backend; keep it out of source control, terminal transcripts, screenshots and browser code.
